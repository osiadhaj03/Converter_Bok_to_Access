...
$form.Text = "BOK to ACCDB Converter"
...
$lblTitle.Text = "BOK to ACCDB File Converter"
...
$btnConvert.Add_Click({
    ...
    Log-Message "=== Starting conversion process ===" "Yellow"
    ...
    try {
        ...
        for ($i = 0; $i -lt $lstBokFiles.Items.Count; $i++) {
            ...
            try {
                ...
                Log-Message "   Conversion successful" "LimeGreen"
                $filesConverted++
            }
            catch {
        ...
        Log-Message "=== Conversion process completed ===" "Yellow"
        Log-Message "Success: $filesConverted | Skipped: $filesSkipped | Failed: $filesError" "Cyan"
        
        if ($filesConverted -gt 0) {
            Log-Message "Converted files saved in: $($txtOutputFolder.Text)" "LimeGreen"
...
# Initialize form
$form.Add_Load({
    Log-Message "Welcome to BOK to ACCDB Converter" "Yellow"
    Log-Message "Select BOK files and output folder, then click 'Start Convert'" "White"
