...
class ShamelaGUI:
    ...
    def test_connection(self):
        ...
        try:
            # إعداد معاملات الاتصال باستخدام PyMySQL
            connection_params = {
            ...
            messagebox.showinfo("نجح الاتصال", 
                              f"تم الاتصال بقاعدة البيانات بنجاح!\n\n"
                              f"إصدار MySQL: {version}\n"
                              f"قاعدة البيانات: {database}")
            self.log_message("✅ تم اختبار الاتصال بقاعدة البيانات بنجاح باستخدام PyMySQL")
            ...
            if error_code == 2059:
                full_error_msg += "\n\nحلول مقترحة:\n• شغل سكريپت setup_database.sql\n• أو استخدم المستخدم الجديد: shamela/shamela123"
            elif error_code == 1045:
                full_error_msg += "\n\nخطأ في كلمة المرور أو اسم المستخدم"
            elif error_code == 2003:
                full_error_msg += "\n\nتأكد من أن MySQL يعمل على المنفذ المحدد"
            
            messagebox.showerror("فشل الاتصال", full_error_msg)
            self.log_message(f"❌ فشل اختبار الاتصال: {full_error_msg}")
            
        except Exception as e:
            error_msg = f"خطأ عام: {str(e)}"
            messagebox.showerror("فشل الاتصال", error_msg)
            self.log_message(f"❌ فشل اختبار الاتصال: {error_msg}")
